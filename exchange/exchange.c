...
static void print_usage(FILE *out, char *progname) {
    ...
    fatal_fprintf(out, "Atomically exchange two files or folders using the renameat2(2) system call.\n");
    fatal_fprintf(out, "\n");
...
int main(int argc, char *argv[]) {
    ...
    if (syscall(SYS_renameat2, AT_FDCWD, source, AT_FDCWD, dest, RENAME_EXCHANGE) != 0) {
        perror("renameat2");
        exit(EXIT_FAILURE);
    }
}
